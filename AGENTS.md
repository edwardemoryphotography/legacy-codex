...
Read this file first, every session, before touching anything else. It routes to STATE.md, TODOS.md, TRIAGE-STATE.md, NEXT-ACTION-LOG.md, FABLE-LEADER-KIT.md, and AGENT_FEEDBACK.md.
Scope: docs/coordination only for this task family — never touch legacy-codex `index.html` (frozen, see STATE.md § FROZEN).
...
|---|---|---|
| `STATE.md` | High-level shipped / blocked / next snapshot, frozen-file list, stack & key reference | Start of any session, before anything else |
...
**Never touch legacy-codex `index.html`.** It is under a freeze spec per `STATE.md` — no rewrite unless explicitly told "REWRITE THE APP CODE".
