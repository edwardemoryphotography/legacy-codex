...
**Config file note:** The project is on Next.js 16 and uses `next.config.mjs`. Do not create a second Next config file.
...
`CodexApp` owns a single piece of state: `activeTab: TabId`. It renders a fixed/sticky `<nav role="tablist">` and conditionally mounts the matching tab component. Tabs are fully independent — they share no state with each other or with `CodexApp`. Adding a tab requires: (1) adding a `TabId` in `src/types/index.ts`, (2) adding an entry to the `TABS` array in `CodexApp.tsx`, and (3) creating the tab component and wiring it in the conditional render block.
...
`src/lib/supabase/client.ts` creates a browser Supabase client (`@supabase/ssr`'s `createBrowserClient`) against its own project — `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`, falling back to project `pkydkbuodikttfeawqsw` if unset (see `.env.local.example`). **Correction (live-verified 2026-09-06):** no Supabase project named or aliased `supabase-indigo-paddle` exists on this account — `codex-system-architecture`'s own CLAUDE.md self-documents its production Supabase project as `foundry-console` (`pkydkbuodikttfeawqsw`), the same ID this file falls back to. Do not assume the two repos are on separate projects, and do not assume shared tables/credentials either — confirm each app's actual *configured* (not fallback) `NEXT_PUBLIC_SUPABASE_URL` in its Vercel deployment before relying on either claim.
