...
export default function MilestonesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const firstError = ms.error ?? sp.error ?? fr.error ?? ev.error;
      ...
      // Knowledge base lives project-wide (codex_documents), not per
      // workspace. It may be unreadable under some policies — degrade
  ...
  async function insertMilestone(
    ...
    void logEvent(current.id, "milestone.created", "milestone", data.id, {
      title: data.title,
    });
    return data;
  }
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await insertMilestone(title, description, targetDate || null);
      if (!requestGate.isScopeCurrent(workspaceId)) return;
  ...
  async function acceptSignal(s: Signal) {
    ...
    setBusy(true);
    try {
      await insertMilestone(s.title, s.description, s.targetDate);
      if (!requestGate.isScopeCurrent(workspaceId)) return;
            ...
            <div className="relative ml-2.5 space-y-4 border-l border-zinc-800 pl-6">
              {milestones.map((m) => {
                ...
                const related = events.filter(
                  (ev) =>
                    ev.target_type === "milestone" && ev.target_id === m.id
                );
